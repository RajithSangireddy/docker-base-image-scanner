...
#aws ecr wait image-scan-complete --repository-name centos --image-id imageTag=25
#if [ $(echo $?) -eq 0 ]; then
  SCAN_FINDINGS=$(aws ecr describe-image-scan-findings --repository-name centos --image-id imageTag=32 | jq '.imageScanFindings.findingSeverityCounts')
  ...
  if [ $CRITICAL != null ] || [ $HIGH != null ]; then
    ...
    aws ecr batch-delete-image --repository-name centos --image-ids imageTag=32  #delete pushed image from container registry
